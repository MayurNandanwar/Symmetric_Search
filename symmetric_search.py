...
SERVICE_ACCOUNT_FILE = './token_cred_new.json'
...
# 🔹 Enable Flask-Session with File-Based Storage
app.config["SECRET_KEY"] = os.environ.get("FLASK_SECRET_KEY", app.secret_key)
app.config["SESSION_TYPE"] = "redis"  # Store sessions in files
#app.config["SESSION_FILE_DIR"] = "/home/bharat/AI_Agent/ai_question_answer/Symmetric_Search_Bot/flask_sessions"  # Folder to store session files
app.config["SESSION_PERMANENT"] = False  # Expire session when browser closes
app.config["SESSION_USE_SIGNER"] = True  # Sign session cookies for security
app.config['SESSION_KEY_PREFIX'] = 'flask_session:'  # Prefix for Redis keys
#app.config["SESSION_COOKIE_HTTPONLY"] = True  # Prevent JavaScript access
#app.config["SESSION_COOKIE_SECURE"] = False  # Change to True for HTTPS
app.config["SESSION_REDIS"] = redis.StrictRedis(host="128.91.51.73", port=6379, db=0,decode_responses=False)  # Connect to Redis
...
REQUEST_COUNT = Counter('flask_app_requests_total', 'Total requests by method and endpoint', ['method', 'endpoint'])
...
def sql_connection():
    ...
    connection_string = 'postgres://postgres:postgres@128.91.51.73:5432/chatbotdb'
        ...
        if request.method == 'POST':
            ...
            if len(rows) == 0:
                ...
                if msg == 0:
                    ...
                    REQUEST_COUNT.labels(method=request.method, endpoint='/signup').inc()
...
def chatpage():
    ...
    REQUEST_COUNT.labels(method=request.method, endpoint='/chatpage').inc()
...
def get_ans_from_csv():
    ...
    try:
        ...
        doc_file_lst = file_name.split(' ')
        final_file_name = '_'.join(doc_file_lst)
        table_name = final_file_name.split('.')[0]
        ...
        if query_text :
            if not file_name or file_name == "Select a document":
                ...
                que_asked = {}
                que_asked['question'] = query_text
                sql_query = """select * from  public.user_questions where email_id = %s AND file_name = %s;"""
                conn = sql_connection()
                cursor = conn.cursor()
                cursor.execute(sql_query,(email,file_name))
                results = cursor.fetchall()

                if len(results)==0:
                    que_asked_lst = [que_asked]

                    # Insert new feedback
                    que_asked_lst_json = json.dumps(que_asked_lst)  # Store as list

                    sql_query = sql_query = """INSERT INTO public.user_questions (email_id, file_name,user_question)
                                                VALUES (%s, %s, %s);"""


                    cursor.execute(sql_query, (email, file_name,que_asked_lst_json))
                    conn.commit()
                    conn.close()

                else:
                    que_asked_lst = results[0][-1]
                    que_asked_lst.append(que_asked)  # Append new feedback
                    que_asked_lst_json = json.dumps(que_asked_lst)  # Convert back to JSON

                    sql_query = """UPDATE public.user_questions SET user_question = %s
                                WHERE email_id = %s AND file_name = %s;"""


                    cursor.execute(sql_query, (que_asked_lst_json, email, file_name))
                ...
                # Retrieve the most similar question from the database
                similarity_score, similar_vector, ans = get_most_similar_question(model,doc_id, file_name,table_name, query_embedding, top_k=1)
                if similarity_score >= 0.8:
                    ...
                    if similar_question:
                        question_part = "We found multiple matching questions! Let’s get specific—select the one that fits your needs."
                    ...
                    sql_query = """select * from  public.user_feedback where email_id = %s AND file_name = %s;"""
                    conn = sql_connection()
                    cursor = conn.cursor()
                    cursor.execute(sql_query,(email,file_name))
                    results = cursor.fetchall()
                    ...
                    ans_not_found_dict = {}
                    ans_not_found_dict['question'] = query_text
                    ans_not_found_dict['answer'] = 'Not Found.'
                    if len(results)==0:
                        ans_not_found = [ans_not_found_dict]

                        # Insert new feedback
                        ans_not_found_json = json.dumps(ans_not_found)  # Store as list

                        sql_query = sql_query = """INSERT INTO public.user_feedback (email_id, file_name, feedback,ans_not_found)
                                                        VALUES (%s, %s, %s,%s);"""

                        cursor.execute(sql_query, (email, file_name, None, ans_not_found_json))
                        conn.commit()
                        conn.close()
                    else:
                        if results[0][-1] is None:
                            ans_not_found_json = json.dumps([ans_not_found_dict])
                        else:
                            ans_lst = results[0][-1]
                            ans_lst.append(ans_not_found_dict)  # Append new feedback
                            ans_not_found_json = json.dumps(ans_lst)  # Convert back to JSON

                        sql_query = """UPDATE public.user_feedback SET ans_not_found = %s
                                    WHERE email_id = %s AND file_name = %s;"""
                        cursor.execute(sql_query, (ans_not_found_json, email, file_name))
                        conn.commit()
                        conn.close()

                    answer = "No results found! Try searching with different keywords.(help me with your feedback to make me more mature)"
                REQUEST_COUNT.labels(method=request.method, endpoint='/ask').inc()
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if str(session.get('token')) == str(entered_token):
                ...
                if table_created==0:
                    datetime = time.ctime()
                    sql_query = "INSERT INTO public.register_table (username, password, email_id,datetime) VALUES (%s, %s, %s,%s);"
                    curr = connection.cursor()
                ...
                gc.collect()
                REQUEST_COUNT.labels(method=request.method, endpoint='/validate_mail').inc()
                return redirect(url_for('login_page'))
            else:
...
def validate_mail_reset_password():
    ...
    if request.method == 'POST':
        ...
        if str(session.get('reset_token')) == str(entered_token):
            REQUEST_COUNT.labels(method=request.method, endpoint='/validate_mail_reset_password').inc()
    ...
    try:
        if request.method == 'POST':
            ...
            # Check password strength using regex
                ...
                gc.collect()
                REQUEST_COUNT.labels(method=request.method, endpoint='/reset_password').inc()
                return redirect(url_for('login_page'))
        return render_template('reset_password.html')
    except PgConnectionError as exe:
        return jsonify({'error':str(exe)})
        ...
        if request.method == 'POST':
            ...
            if len(rows) == 0:
                ...
                if msg == 0:
                    ...
                    REQUEST_COUNT.labels(method=request.method, endpoint='/forgot_password').inc()
...
def clear():
    ...
    REQUEST_COUNT.labels(method=request.method, endpoint='/clear').inc()
...
def save_feedback():
    ...
    if len(results)==0:
        ...
        sql_query = """INSERT INTO public.user_feedback (email_id, file_name, feedback,ans_not_found)
                       VALUES (%s, %s, %s,%s);"""
        with sql_connection() as conn:
            with conn.cursor() as curr:
                curr.execute(sql_query, (email, file_name, feedback_json,None))
                conn.commit()
    else:
        if results[0][-2] is None:
            feedback_json = json.dumps([feedback_res])
        else:
        ...
        with sql_connection() as conn:
            with conn.cursor() as curr:
                ...
                conn.commit()
    REQUEST_COUNT.labels(method=request.method, endpoint='/save_feedback').inc()
    return jsonify({'message': 'Feedback saved successfully'}), 200


def extract_api_counts(metrics_string):
    # Dictionary to store counts by endpoint and method
    api_counts = defaultdict(lambda: defaultdict(float))

    # Regex pattern to find 'flask_app_requests_total' entries
    total_pattern = re.compile(r'flask_app_requests_total\{endpoint="([^"]+)",method="([^"]+)"\} (\d+\.\d+)')

    # Iterate through total matches and sum counts for each endpoint and method
    for match in total_pattern.finditer(metrics_string):
        endpoint, method, count = match.groups()
        api_counts[endpoint][method] += float(count)
...
def count_api():
    ...
    with open(os.path.join(dir_path,'api_count.json'),mode='w') as f:
        json.dump(count_dicti,f)

    return count_dicti

# Initialize the scheduler
scheduler = BackgroundScheduler()
scheduler.add_job(count_api, 'cron', hour=0, minute=0)  # Runs daily at 12:00 AM
scheduler.start()
# Manual Count Endpoint
@app.route('/count')
def count():
    count_dicti = count_api()
    print("count_dict::",count_dicti)
    return render_template('chart.html', api_counts=count_dicti)


if __name__=='__main__':
    #app.run(ssl_context=('./cert.pem', './key.pem'),host='0.0.0.0',port=5012)
    app.run()